...
# Reusing the functions from view_ctx.py
def decompress_lzw(compressed_data, width, height):
    """
    LZW decompression for ctx files
...
def convert_ctx_to_png(ctx_path, output_dir=None, overwrite=False, gamma=1.0, contrast=1.0, brightness=0):
    """Convert a ctx file to a PNG file"""
    try:
        # Determine output path
        if output_dir:
            # Create relative path within output directory
            rel_path = os.path.relpath(ctx_path, start=args.input_dir)
            output_path = os.path.join(output_dir, os.path.splitext(rel_path)[0] + '.png')
            
            # Create directory if it doesn't exist
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
        else:
            # Use same directory as ctx
            output_path = os.path.splitext(ctx_path)[0] + '.png'
            
        # Skip if file exists and overwrite is False
        if os.path.exists(output_path) and not overwrite:
            print(f"Skipping {ctx_path} (output already exists)")
            return False
        ...
        with open(ctx_path, 'rb') as f:
            ...
            # if os.path.basename(ctx_path).startswith('F'):
        ...
        #     print(f"Warning: Decompression incomplete for {ctx_path}. Expected {width*height} bytes, got {len(image_data) if image_data else 0}")
        ...
        # save image_data to file raw
        with open(output_path.replace('.png', '.raw'), 'wb') as raw_file:
        ...
        # save img_array to file raw
        with open(output_path.replace('.png', '.img.raw'), 'wb') as img_file:
        ...
        # print(f"Converted: {ctx_path} -> {output_path}")
        return True
        
    except Exception as e:
        print(f"Error processing {ctx_path}: {e}")
        return False

def find_and_convert_ctx_files(input_dir, output_dir=None, overwrite=False, max_workers=None, 
                             gamma=1.0, contrast=1.0, brightness=0):
    """Find all ctx files in the input directory recursively and convert them to PNG"""
    
    # Find all ctx files
    ctx_files = []
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.lower().endswith('.ctx'):
                ctx_files.append(os.path.join(root, file))
    
    if not ctx_files:
        print(f"No ctx files found in {input_dir}")
        return
    ...
    print(f"Conversion complete: {success_count} of {len(ctx_files)} files converted successfully")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Convert ctx files to PNG recursively')
    parser.add_argument('input_dir', help='Input directory containing ctx files')
